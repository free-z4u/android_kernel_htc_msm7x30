...
static int rpcrouter_smd_remote_probe(struct platform_device *pdev)
{
	...
	/* Open up SMD channel */
	rc = smd_open("SMD_RPCCALL", &smd_remote_xprt.channel, NULL,
		      rpcrouter_smd_remote_notify);		/* Modified by Andy for HTC SMD upper layer drivers */
	if (rc < 0)
		return rc;
...
static struct platform_driver rpcrouter_smd_remote_driver = {
	...
	.driver		= {
			.name	= "SMD_RPCCALL",	/* Modified by Andy for HTC SMD upper layer drivers */
			.owner	= THIS_MODULE,
	},
};
