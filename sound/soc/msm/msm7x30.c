				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	if (start) {
		printk(KERN_INFO "[ALSA] msm_start_voice");
		broadcast_event(AUDDEV_EVT_START_VOICE, DEVICE_IGNORE,
							SESSION_IGNORE);
	} else {
		printk(KERN_INFO "[ALSA] msm_end_voice");
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	printk(KERN_INFO "[ALSA] msm_set_voice_tx_mute: mute %d\n", mute);
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	printk(KERN_INFO "[ALSA] msm_set_voice_rx_vol: volume %d\n", volume);
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	printk(KERN_INFO "[ALSA] msm_set_volume: volume %d\n", volume);
	...
	printk(KERN_INFO "[ALSA] msm_set_volume: vsession_id %d, volume %d", session_id, volume);
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	/* if (!set)
		return -EPERM; */
	printk(KERN_INFO "[ALSA] msm_route_voice: "
		"tx %d, rx %d, set %d\n",
	...
	if (set) {
		printk(KERN_INFO "[ATS][phonecall_start][successful]\n");
	} else {
		printk(KERN_INFO "[ATS][phonecall_end][successful]\n");
	}
	...
	if (IS_ERR(rx_dev_info)) {
		printk(KERN_ERR "pass invalid dev_id\n");
		rc = PTR_ERR(rx_dev_info);
		return rc;
	}

	if (!(rx_dev_info->capability & SNDDEV_CAP_RX)) {
		printk(KERN_ERR "First Dev is supposed to be RX\n");
		return -EFAULT;
	}
	...
	if (!(tx_dev_info->capability & SNDDEV_CAP_TX)) {
		printk(KERN_ERR "Second Dev is supposed to be Tx\n");
		return -EFAULT;
	}
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	printk(KERN_INFO "device %s set %d\n", dev_info->name, set);
	printk(KERN_INFO "[ALSA] msm_en_device (dev %s, id %d, enable %d, opened %d)\n",
	...
	if (set) {
		if (!dev_info->opened) {
			...
			printk(KERN_INFO "device freq =%d\n", set_freq);
			rc = dev_info->dev_ops.set_freq(dev_info, set_freq);
			if (rc < 0) {
				printk(KERN_ERR "device freq failed!\n");
				return rc;
			}
			...
			if (rc < 0) {
				printk(KERN_ERR "Enabling %s failed, rc=%d\n", dev_info->name, rc);
				return rc;
			}
			dev_info->opened = 1;
			printk(KERN_INFO "set device %s opened as %d\n",
		...
		if (dev_info->opened) {
			...
			if (rc < 0) {
				printk(KERN_ERR "Snd device failed close!\n");
				return rc;
			} else {
				dev_info->opened = 0;
				printk(KERN_INFO "set device %s opened as %d\n",
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	printk(KERN_INFO "[ALSA] msm_route_stream: session %d, dev %d, enable %d\n",
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	if (IS_ERR(dev_info)) {
		rc = PTR_ERR(dev_info);
		printk(KERN_ERR "audio_dev_ctrl_find_dev failed. %ld\n",
				PTR_ERR(dev_info));
		return rc;
	}
	...
	if (dev_info->dev_ops.set_device_volume)
		...
		printk(KERN_INFO "device %s does not support device volume "
...
static int snd_dev_ctl_index(int idx)
{
	...
	if (idx < 0 || idx >= AUDIO_DEV_CTL_MAX_DEV) {
		printk(KERN_ERR "pass invalid idx\n");
		return -EINVAL;
	}
